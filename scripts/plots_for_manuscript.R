...
lags_long$time_lag <- factor(lags_long$time_lag,
                             levels = c("transientT0", "transientT1", "transientT2",
                                        "transientT3", "transientT4", "transientT5",
                                        "transientT6", "transientT7", "transientT8",
                                        "transientT9", "transientT10", "transientT11"),
                             labels = c("NDVI[t+1]", "NDVI[t+2]", "NDVI[t+3]",
                                        "NDVI[t+4]", "NDVI[t+5]", "NDVI[t+6]",
                                        "NDVI[t+7]", "NDVI[t+8]", "NDVI[t+9]",
                                        "NDVI[t+10]", "NDVI[t+11]", "NDVI[t+12]"))

(plot1 <- ggplot(data = lags_long, aes(x = round(NDVI,2), y = round(d.NDVI, 2))) + 
...
quadrant_means_long$quadrant <- as.factor(quadrant_means_long$quadrant)
levels(quadrant_means_long$quadrant) <- c("Phase 1", "Phase 2", "Phase 3", "Phase 4")
quadrant_means_long <- arrange(quadrant_means_long, quadrant)

(plot2 <- ggplot(quadrant_means_long, aes(time_lag, mean_transients, 
                                color = quadrant, 
                                group = quadrant)) + 
  geom_point(size = 2) +
  geom_smooth() +
...
ggplot(NDVI_transient) +
  ...
  scale_y_continuous(sec.axis = dup_axis(name = "NDVI")) +
  facet_wrap(vars(community), scales = "free", nrow = 4) +
  scale_color_manual("",
                     breaks = c("NDVI", "Transients"),
                     values = c("#19AD10", "black")) +
  ylab('Transient Relative Abundance') +
  xlab('Date') +
  theme_bw() +
  scale_x_date(date_breaks = "2 years", date_labels = "%Y") +
  theme(axis.title.y = element_text(),
        legend.position = "top")
#ggsave("plots/ms_plots/timeseries_by_community.png", dpi = 600)

### ACF and CCF ### ------------------------------------------------------------

new_df <- NDVI_transient[-c(1:50, 442:523),] %>% 
  ungroup() %>% 
  select(date, trans_rel_abund, ndvi)

NDVI <- new_df$ndvi
Transient_Rel_Abund <- new_df$trans_rel_abund

par(mfrow = c(3,1))
acf(NDVI, lag.max = 15, main = "NDVI Autocorrelation")
